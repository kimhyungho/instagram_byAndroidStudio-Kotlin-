...
class HyungStagramPostListActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        lateinit var glide: RequestManager

        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_hyung_stagram_post_list)
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        val adapter = PostAdapter(
                            postList!!,
                            LayoutInflater.from(this@HyungStagramPostListActivity),
                            glide
                        )
                        post_recyclerview.adapter = adapter
                        post_recyclerview.layoutManager =
                            LinearLayoutManager(this@HyungStagramPostListActivity)
                    }
                }
            }
        )
        user_info.setOnClickListener {
            startActivity(
                Intent(
                    this@HyungStagramPostListActivity,
